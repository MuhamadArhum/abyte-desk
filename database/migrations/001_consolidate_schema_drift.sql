-- ============================================================
-- Migration 001: Consolidate Schema Drift
-- Removes the need for scattered ALTER TABLE calls in controllers.
-- Run once per tenant database on upgrade.
...
-- ── MISSING INDEXES ──────────────────────────────────────────
-- sale_details: critical for JOIN performance in sales reports
ALTER TABLE sale_details ADD INDEX IF NOT EXISTS idx_sale_details_sale_id (sale_id);
ALTER TABLE sale_details ADD INDEX IF NOT EXISTS idx_sale_details_product_id (product_id);

-- users: login and branch filtering
...
-- credit_sales: overdue queries
ALTER TABLE credit_sales ADD INDEX IF NOT EXISTS idx_credit_due_date (due_date);

-- audit_logs: user-specific timeline queries
ALTER TABLE audit_logs ADD INDEX IF NOT EXISTS idx_audit_user_date (user_id, created_at);

-- ── CHECK CONSTRAINTS (MariaDB 10.2.1+ / MySQL 8.0.16+) ──────
-- Wrapped in procedure to handle databases that already have them
DROP PROCEDURE IF EXISTS add_check_constraints;
DELIMITER $$
CREATE PROCEDURE add_check_constraints()
BEGIN
    -- products price
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.CHECK_CONSTRAINTS
        WHERE CONSTRAINT_SCHEMA = DATABASE() AND CONSTRAINT_NAME = 'chk_product_price'
    ) THEN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.CHECK_CONSTRAINTS
        WHERE CONSTRAINT_SCHEMA = DATABASE() AND CONSTRAINT_NAME = 'chk_sd_quantity'
    ) THEN
        ALTER TABLE sale_details ADD CONSTRAINT chk_sd_quantity CHECK (quantity > 0);
    END IF;

    -- expenses amount
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.CHECK_CONSTRAINTS
        WHERE CONSTRAINT_SCHEMA = DATABASE() AND CONSTRAINT_NAME = 'chk_expense_amount'
    ) THEN
        ALTER TABLE expenses ADD CONSTRAINT chk_expense_amount CHECK (amount > 0);
    END IF;
END $$
DELIMITER ;
CALL add_check_constraints();
DROP PROCEDURE IF EXISTS add_check_constraints;
